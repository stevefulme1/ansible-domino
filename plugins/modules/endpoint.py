...
"""Ansible module for managing Domino Data Lab endpoint resources."""
...
---
module: endpoint
short_description: Manage Domino model API endpoints
description:
    - Create, update, and delete Domino Data Lab endpoint resources.
version_added: "1.0.0"
author:
...
- name: Create a endpoint
  stevefulme1.domino.endpoint:
    api_url: "https://app.dominodatalab.com"
    api_key: "my-api-key"
    model_id: "example"
    state: present

- name: Delete a endpoint
  stevefulme1.domino.endpoint:
    api_url: "https://app.dominodatalab.com"
    api_key: "my-api-key"
    model_id: "example"
...
RETURN = r"""
endpoint:
    description: The endpoint resource details.
...
def create_resource(client, params):
    ...
    return client.post("/api/v1/endpoints", data=payload)


def update_resource(client, existing, params):
    """Update an existing resource."""
    resource_id = existing.get("id", existing.get("endpoint_id", ""))
    payload = {k: v for k, v in params.items() if v is not None and k not in (
        "api_url", "api_key", "validate_certs", "timeout", "state",
    )}
    return client.put(f"/api/v1/endpoints/{resource_id}", data=payload)


def delete_resource(client, existing):
    """Delete a resource."""
    resource_id = existing.get("id", existing.get("endpoint_id", ""))
    client.delete(f"/api/v1/endpoints/{resource_id}")


def needs_update(params, existing):
    """Check if existing resource needs updating."""
    check_fields = ['name', 'environment_id']
...
def main():
    ...
    if existing is None:
        ...
        module.exit_json(changed=True, endpoint=resource)
        return
    ...
    module.exit_json(changed=False, endpoint=existing)
