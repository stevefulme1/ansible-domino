...
"""Ansible module to query Domino Data Lab job resources."""
...
---
module: job_info
short_description: List or retrieve Domino jobs
description:
    - Retrieve information about Domino Data Lab job resources.
version_added: "1.0.0"
author:
...
options:
    ...
    job_id:
        description:
            - The job id for the Domino Data Lab resource.
        type: str
...
def main():
    ...
    if params.get("project_id"):
        result = client.get(f"/api/v1/jobs/{params['project_id']}")
        module.exit_json(changed=False, jobs=[result] if result else [])
    else:
        result = client.get("/api/v1/jobs")
        items = result if isinstance(result, list) else result.get("data", result.get("items", []))
        module.exit_json(changed=False, jobs=items)
